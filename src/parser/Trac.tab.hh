...
#ifndef YY_YY_TRAC_TAB_HH_INCLUDED
# define YY_YY_TRAC_TAB_HH_INCLUDED
...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    union union_type
    {
      ...
      // calls
      char dummy8[sizeof (vector<Call*>)];

      // vars
      char dummy9[sizeof (vector<shared_ptr<Variable>>)];
        ...
        switch (this->kind ())
    {
      ...
      case symbol_kind::S_trac: // trac
        value.move< Trac* > (std::move (that.value));
        break;
      ...
      case symbol_kind::S_variable: // variable
        value.move< shared_ptr<Variable> > (std::move (that.value));
        break;
      ...
      case symbol_kind::S_calls: // calls
        value.move< vector<Call*> > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, Trac*&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const Trac*& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, shared_ptr<Variable>&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const shared_ptr<Variable>& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, vector<Call*>&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const vector<Call*>& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yykind)
    {
      ...
      case symbol_kind::S_variable: // variable
        value.template destroy< shared_ptr<Variable> > ();
        break;
      ...
      case symbol_kind::S_calls: // calls
        value.template destroy< vector<Call*> > ();
        break;
      ...
      {
        return TracParser::symbol_name (this->kind ());
      }
#endif // #if YYDEBUG || 0
    ...
    struct symbol_type : basic_symbol<by_kind>
    {
...
#endif
      {
        ...
        YY_ASSERT (tok == token::END
                   ...
                   || (token::LPAREN <= tok && tok <= token::VOID));
#endif
      }
#if 201103L <= YY_CPLUSPLUS
    ...
    TracParser (TracScanner &scanner_yyarg, TracReader &reader_yyarg, TracResult &result_yyarg);
    virtual ~TracParser ();

#if 201103L <= YY_CPLUSPLUS
    /// Non copyable.
    TracParser (const TracParser&) = delete;
    /// Non copyable.
    TracParser& operator= (const TracParser&) = delete;
#endif
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_INTEGER (long v, location_type l)
      {
      ...
      static
      symbol_type
      make_INTEGER (const long& v, const location_type& l)
      {
...
#if YY_CPLUSPLUS < 201103L
    /// Non copyable.
    TracParser (const TracParser&);
    /// Non copyable.
    TracParser& operator= (const TracParser&);
#endif
...
#line 1657 "Trac.tab.hh"




#endif // !YY_YY_TRAC_TAB_HH_INCLUDED
